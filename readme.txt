...
刚才又试了试把邮箱改成一个不存在的邮箱，结果证明邮箱不存在是不能push代码的。客户端有时显示成功，有时也会显示失败。估计这是一个bug。不管客户端怎样显示，在远程仓库里都不会看到更新被提交。只有切换成正确的邮箱后再次提交才会显示成功。而且诡异的是，这时候远程仓库显示的提交人是之前用错误邮箱提交时的用户名~


呵呵，在gitee上边好像不需要正确的邮箱诶